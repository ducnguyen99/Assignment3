...
# Create data loader for training with batch size 64 and shuffling
...
# Instantiate the API object (which includes rate limiting)
api = API()
query_data = []
query_response = []
num_query = 100
for i, (data, _) in enumerate(train_loader):
    with torch.no_grad():
        response = api.handler(model, data, rounding=True)
...
query_response = torch.cat(query_response, dim=0)  
...
# query = []
# Loop over the training data batches from the train_loader
# for batch_idx, (data, target) in enumerate(train_loader):
#     with torch.no_grad():  # Disable gradient computation for efficiency
#         try:
#             # Query the target model via the API (e.g., for model extraction or distillation)
#             # The API applies rate limiting and returns rounded logits
#             target_response = api.handler(model, data, rounding=True)

#             # Store the input data and the model's response as a tuple
#             query.append((data, target_response))
#         except Exception as e:
#             # If the API rate limit is reached, print the error and break the loop
#             print(e)
#             break

#     # Stop after a fixed number of batches (to enforce a hard limit on queries)
#     if batch_idx == num_query:
#         break
...
optimizer = optim.Adam(extract_model.parameters()) # depend on 
    ...
    for batch_idx, (data, target) in enumerate(query_loader):
        ...
        if batch_idx % 10 == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                epoch, batch_idx * len(data), len(query_loader.dataset),
