...
def train_model(model, train_loader, epochs=5, normalization=False, smothing=False):
    ...
    if smothing: criterion = nn.CrossEntropyLoss(label_smoothing=0.1)
...
for _ in range(num_shadow_models):
    ...
    shadow_train_imgs, shadow_train_labels = select_high_confidence_samples(target_model, synthetic_inputs, threshold=0.2, max_samples=shadow_train_size)
    
    # Simulate separate shadow test set
    synthetic_inputs = generate_random_images(10000)
    shadow_test_imgs, shadow_test_labels = select_high_confidence_samples(target_model, synthetic_inputs, threshold=0.2, max_samples=shadow_test_size)
